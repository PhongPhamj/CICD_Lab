...
pipeline {
    ...
    environment {
        ...
        REPO_NAME = 'cicd-lab'
        SONARQUBE_ANALYSIS_URL = 'http://13.236.123.33:9000/dashboard?id=com.example%3AbackendCICD'
 ...
 ******************************************************************************/
        ...
        stage('Scan Image') {
            steps {
                ...
                // }
                sh '''
                    trivy image --db-repository public.ecr.aws/aquasecurity/trivy-db:2 --no-progress --exit-code 1 --severity HIGH,CRITICAL -f json -o trivy-report.json $DOCKER_HUB_USERNAME/$REPO_NAME:latest
                '''
                // sh '''
